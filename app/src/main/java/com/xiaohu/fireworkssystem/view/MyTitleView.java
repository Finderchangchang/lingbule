...
public class MyTitleView extends RelativeLayout {
    ...
    private void setupAttributes(AttributeSet attrs) {
        ...
        if (leftid != R.mipmap.ic_launcher) {
            leftImage.setVisibility(VISIBLE);
        }
        if (rightid != R.mipmap.ic_launcher) {
...
//                    R.styleable.CustomView_displayShapeName, false);
    ...
    private Bitmap getBitmeap(int Rid) {
        return imageDemoTest.decodeSampledBitmapFromResourse(Rid, 30, 30);
    }
    public void setTitleName(String n){
        name.setText(n);
    }
