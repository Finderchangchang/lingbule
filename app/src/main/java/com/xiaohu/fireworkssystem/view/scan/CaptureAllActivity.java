package com.xiaohu.fireworkssystem.view.scan;
...
public class CaptureAllActivity extends Activity {
...
private Button btnClose;
	...
	private void findViewById() {
		...
		btnClose= (Button) findViewById(R.id.btn_scan_close);
	}
	...
	PreviewCallback previewCb = new PreviewCallback() {
		public void onPreviewFrame(byte[] data, Camera camera) {
			...
			if (!TextUtils.isEmpty(resultStr)) {
				...
				scanResult.setText("条形码" + resultStr);

				Intent intent=new Intent();
				if (resultStr.length()>=9){
					resultStr = resultStr.substring(resultStr.length()-9,resultStr.length());
					intent.putExtra("BarScan",resultStr);
					setResult(15,intent);
					barcodeScanned = true;
				}else {
					barcodeScanned = false;
