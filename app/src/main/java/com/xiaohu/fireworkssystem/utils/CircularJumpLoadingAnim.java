...
public class CircularJumpLoadingAnim extends View {
//loading动画
    ...
    public CircularJumpLoadingAnim(Context context) {
        this(context, null);
    }

    public CircularJumpLoadingAnim(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CircularJumpLoadingAnim(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < circularCount; i++) {
            if (i == mJumpValue % circularCount) {
                canvas.drawCircle(i * circularX + circularX / 2f,
                        mHeight / 2 - mHeight / 2 * mAnimatedValue,
                        mMaxRadius, mPaint);
            } else {
                canvas.drawCircle(i * circularX + circularX / 2f,
                        mHeight / 2,
